...
function candyCrushGame() {
    ...
    const swapSound = new Audio("music/swap.mp3");
    const matchSound = new Audio("music/match.mp3");
    bgMusic.volume = 0.4; // optional (so it's not too loud)
    ...
    function checkRowForFour() {
        for (let i = 0; i < 60; i++) {
            ...
            if (rowOfFour.every(index => squares[index].style.backgroundImage === decidedColor && !isBlank)) {
                ...
                showScorePopup(300, 300, 4);
                
                rowOfFour.forEach(index => {
                    squares[index].classList.add("match");
    setTimeout(() => {
        squares[index].style.backgroundImage = "";
        squares[index].classList.remove("match");
    }, 300);
});
    ...
    function checkRowForThree() {
        for (let i = 0; i < 62; i++) {
            ...
            if (rowOfThree.every(index => squares[index].style.backgroundImage === decidedColor && !isBlank)) {
                score += 3;
                scoreDisplay.innerHTML = score;
                showScorePopup(300, 300, 3);
                
                    rowOfThree.forEach(index => {
    squares[index].classList.add("match"); // 💥 animation
    setTimeout(() => {
        squares[index].style.backgroundImage = "";
        squares[index].classList.remove("match");
    }, 300);
...
function showScorePopup(x, y, value) {
    ...
    popup.classList.add("floating-score");
